...
class Utilities(ToolCollection):
    ...
    def move_all_objects_into_view(self) -> str:
        ...
        try:
            ...
            # The simplest approach is to call fit() on the active viewport
            try:
                vp = app.activeViewport
                vp.fit()
                return json.dumps({"status": "Success", "message": "All objects moved into view (camera fit)."})
            except Exception as e:
                return json.dumps({"error": f"Failed to fit camera: {str(e)}"})
